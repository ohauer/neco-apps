...
func testMeows() {
	...
	It("should deploy runner pool", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Eventually(func() error {
			stdout, stderr, err := ExecAt(boot0, "kubectl", "--namespace", "meows-runner",
				"get", "deployment/runnerpool-"+hostname, "-o=json")
			if err != nil {
				return fmt.Errorf("stdout: %s, stderr: %s, err: %v", stdout, stderr, err)
			}
			deployment := new(appsv1.Deployment)
			...
			if deployment.Status.AvailableReplicas != 1 {
				return fmt.Errorf("AvailableReplicas is not 1: %d", deployment.Status.AvailableReplicas)
			}
			return nil
		}).Should(Succeed())
